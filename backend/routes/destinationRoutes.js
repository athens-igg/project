...
router.get("/search", async (req, res) => {
  ...
  try {
    ...
    //const apiUrl = `https://api.opentripmap.com/0.1/en/places/radius?radius=5000&lon=${lon}&lat=${lat}&limit=10&format=json&apikey=${apiKey}`;
    ...
    if (!response.data || !response.data.features) {
      return res
        .status(500)
        .json({ message: "Invalid response from OpenTripMap API" });
    }
    ...
    const destinations = response.data.features.map((feature) => ({
      ...
      category: feature.properties.kinds
        ? feature.properties.kinds.split(",")[0]
        : "N/A",
...
router.get("/search", async (req, res) => {
  ...
  try {
    ...
    // Step 2: Fetch places using lat & lon with proper format=json
    const apiUrl = `https://api.opentripmap.com/0.1/en/places/radius?radius=5000&lon=${lon}&lat=${lat}&limit=10&format=json&apikey=${apiKey}`;
    console.log("🔍 Fetching places from:", apiUrl);
    ...
    if (!Array.isArray(response.data) || response.data.length === 0) {
      return res
        .status(404)
        .json({ message: "No places found for the given location" });
    }

    // Step 3: Extract destination details
    const destinations = response.data.map((place) => ({
      name: place.name || "Unknown",
      latitude: place.point.lat,
      longitude: place.point.lon,
      category: place.kinds ? place.kinds.split(",")[0] : "N/A",
